...
namespace AbstractProtocol.AnonymousPipeComplex {
	public class AppProtocol {
		...
		public static Protocol Build() {
			...
			return new ProtocolBuilder()
				...
				.Messages
					...
					.For<RequestListFolder>(AppProtocolMessageType.RequestListFolder).SerializeWith(new BinaryFormattedSerializer<RequestListFolder>())
		...
		private static HandshakeOutcome VerifyHandshake(ProtocolOrchestrator orchestrator, Sync handshake, Ack response, out Verack verack) {
			// here the client
		...
		private static bool AcknowledgeHandshake(ProtocolOrchestrator orchestrator, Sync handshake, Ack response, Verack verack) {
			// unncessary
			return true;
		}
		...
		private static FolderContents ListFolder(ProtocolOrchestrator orchestrator, RequestListFolder request) {
			...
			var files = Directory.GetFiles(request.Folder).Select(f => new FileDescriptor(new FileInfo(f))).Cast<Descriptor>();
			var subDirs = Directory.GetDirectories(request.Folder).Select(d => new DirectoryDescriptor(new DirectoryInfo(d))).Cast<Descriptor>();
			return new FolderContents {
				Items = files.Union(subDirs).ToArray()
			};
		}

		private static void PrintFolderContents(ProtocolOrchestrator orchestrator, RequestListFolder request, FolderContents response) {
			SystemLog.Info($"PrintFolderContents: Request: {request}, Respose {response} ");
		}

		private static void GetFilePart(ProtocolOrchestrator orchestrator, RequestListFolder request, FolderContents response) {
