		...
		public static async Task Main(string[] args) {
            ...
            try {
                ...
                var endpoint1 = Task.Run(async () => {
                    ...
                    orchestator.StateChanged += (state) => SystemLog.Info($"[SERVER] Orchestration state changed to {state}");
                    orchestator.MessageError += (queue, envelope, error) => SystemLog.Error($"[SERVER] Queue: {queue}, Message: {envelope}, Error: {error.ToDiagnosticString()}");
                ...
                var endpoint2 = Task.Run(async () => {
                    ...
                    orchestator.StateChanged += (state) => SystemLog.Info($"[CLIENT] Orchestration state changed to {state}");
                    orchestator.MessageError += (queue, envelope, error) => SystemLog.Error($"[CLIENT] Queue: {queue}, Message: {envelope}, Error: {error.ToDiagnosticString()}");
