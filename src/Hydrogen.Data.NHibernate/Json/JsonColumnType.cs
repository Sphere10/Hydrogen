...
public class JsonColumnType<T> : IUserType where T : class {
	...
	public T Deserialize(string jsonString) => string.IsNullOrWhiteSpace(jsonString) ? CreateObject(typeof(T)) : JsonWorker.Deserialize<T>(jsonString);

	public string Serialize(T obj) => obj == null ? "{}" : JsonWorker.Serialize(obj);
