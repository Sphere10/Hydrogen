...
public class AnchorArea : ICloneable, IComparable {
	...
	public bool IsEmpty {
		get {
			return HasRight == false && HasLeft == false &&
			       HasTop == false && HasBottom == false &&
			       Middle == false && Center == false;
		}
	}

	[DefaultValue(float.NaN)] public float Right = float.NaN;

	[DefaultValue(float.NaN)] public float Left = float.NaN;

	[DefaultValue(false)] public bool Center;

	[DefaultValue(float.NaN)] public float Top = float.NaN;

	[DefaultValue(float.NaN)] public float Bottom = float.NaN;

	[DefaultValue(false)] public bool Middle;
