...
public struct CellRange {
	...
	public CellRange(Position p_Start, Position p_End) : this(p_Start, p_End, true) {
	}

	public static CellRange FromPosition(Position startPosition) {
		return new CellRange(startPosition);
	}


	public static CellRange From(Position startPosition, int rowCount, int colCount) {
		var range = new CellRange(startPosition);
	...
	public CellRange(int p_StartRow, int p_StartCol, int p_EndRow, int p_EndCol) {
	...
	private CellRange(Position p_Start, Position p_End, bool p_bCheck) {
		m_Start = p_Start;
		m_End = p_End;
	...
	static CellRange() {
		Empty = new CellRange(Position.Empty, Position.Empty, false);
	}
	...
	public int ColumnsCount {
		get { return (m_End.Column - m_Start.Column) + 1; }
		set {
	...
	public int RowsCount {
		get { return (m_End.Row - m_Start.Row) + 1; }
		set {
			if (value <= 0) //facendo questo controllo non serve richiamare Normalize
				throw new SourceGridException("Invalid columns count");
	...
	public CellRange(Position p_SinglePosition) : this(p_SinglePosition, p_SinglePosition, false) {
	}

	/// <summary>
	/// Represents an empty range
	/// </summary>
	public readonly static CellRange Empty;
	...
	/// <param name="p_Range"></param>
	/// <returns></returns>
	public bool Equals(CellRange p_Range) {
	...
	/// <returns></returns>
	public static RangeRegion Union(CellRange p_Range1, CellRange p_Range2) {
	...
	/// <param name="p_Range2"></param>
	/// <returns></returns>
	public static CellRange GetBounds(CellRange p_Range1, CellRange p_Range2) {
		if (p_Range1.IsEmpty())
			return p_Range2;
		else if (p_Range2.IsEmpty())
			return p_Range1;
		else
			return new CellRange(Position.Min(p_Range1.Start, p_Range2.Start),
				Position.Max(p_Range1.End, p_Range2.End),
				false);
	}
	...
	/// <param name="p_Range2"></param>
	/// <returns></returns>
	public static CellRange Intersect(CellRange p_Range1, CellRange p_Range2) {
		...
		if (startNew.Column > endNew.Column ||
			...
			return new CellRange(startNew, endNew, false);
	}
	...
	/// <param name="p_Range"></param>
	/// <returns></returns>
	public CellRange Intersect(CellRange p_Range) {
	...
	/// <param name="p_Range2"></param>
	/// <returns></returns>
	public static bool IntersectsWith(CellRange p_Range1, CellRange p_Range2) {
	...
	/// <param name="p_Range"></param>
	/// <returns></returns>
	public bool IntersectsWith(CellRange p_Range) {
	...
	/// <returns></returns>
	public RangeRegion Exclude(CellRange range) {
		RangeRegion excluded;

		CellRange intersection = Intersect(range);
		if (intersection.IsEmpty()) {
			...
			if (this.Start.Row < intersection.Start.Row &&
				...
				excluded.Add(new CellRange(this.Start.Row, this.Start.Column, intersection.Start.Row - 1, intersection.Start.Column - 1));

			//Top
			if (this.Start.Row < intersection.Start.Row)
				excluded.Add(new CellRange(this.Start.Row, intersection.Start.Column, intersection.Start.Row - 1, intersection.End.Column));

			//Top Right
			if (this.Start.Row < intersection.Start.Row &&
			    this.End.Column > intersection.End.Column)
				excluded.Add(new CellRange(this.Start.Row, intersection.End.Column + 1, intersection.Start.Row - 1, this.End.Column));

			//----------

			//Left
			if (this.Start.Column < intersection.Start.Column)
				excluded.Add(new CellRange(intersection.Start.Row, this.Start.Column, intersection.End.Row, intersection.Start.Column - 1));

			//Right
			if (this.End.Column > intersection.End.Column)
				excluded.Add(new CellRange(intersection.Start.Row, intersection.End.Column + 1, intersection.End.Row, this.End.Column));
			...
			if (this.End.Row > intersection.End.Row &&
			    this.Start.Column < intersection.Start.Column)
				excluded.Add(new CellRange(intersection.End.Row + 1, this.Start.Column, this.End.Row, intersection.Start.Column - 1));

			//Bottom
			if (this.End.Row > intersection.End.Row)
				excluded.Add(new CellRange(intersection.End.Row + 1, intersection.Start.Column, this.End.Row, intersection.End.Column));

			//Bottom Right
			if (this.End.Row > intersection.End.Row &&
			    this.End.Column > intersection.End.Column)
				excluded.Add(new CellRange(intersection.End.Row + 1, intersection.End.Column + 1, this.End.Row, this.End.Column));
		}
...
/// <summary>
/// Interface that rappresent a range of the grid. (RangeGridNoFixedRows, RangeFullGridNoFixedCols, RangeFixedRows, RangeFixedCols, Range)
...
public interface IRangeLoader {
	...
	/// </summary>
	CellRange GetRange(GridVirtual p_grid);
}
...
public class RangeFullGrid : IRangeLoader {
	...
	/// <returns></returns>
	public CellRange GetRange(GridVirtual p_Grid) {
...
public class RangeFullGridNoFixedRows : IRangeLoader {
	...
	public CellRange GetRange(GridVirtual p_Grid) {
		if (p_Grid.Rows.Count >= p_Grid.FixedRows)
			return new CellRange(p_Grid.FixedRows, 0, p_Grid.Rows.Count - 1, p_Grid.Columns.Count - 1);
		else
			return CellRange.Empty;
	}
}
...
public class RangeFullGridNoFixedCols : IRangeLoader {
	...
	public CellRange GetRange(GridVirtual p_Grid) {
		if (p_Grid.Columns.Count >= p_Grid.FixedColumns)
			return new CellRange(0, p_Grid.FixedColumns, p_Grid.Rows.Count - 1, p_Grid.Columns.Count - 1);
		else
			return CellRange.Empty;
	}
}
...
public class RangeFixedRows : IRangeLoader {
	...
	public CellRange GetRange(GridVirtual p_Grid) {
		if (p_Grid.Rows.Count >= p_Grid.FixedRows)
			return new CellRange(0, 0, p_Grid.FixedRows, p_Grid.Columns.Count - 1);
		else
			return CellRange.Empty;
	}
}
...
public class RangeFixedCols : IRangeLoader {
	...
	public CellRange GetRange(GridVirtual p_Grid) {
		if (p_Grid.Columns.Count >= p_Grid.FixedColumns)
			return new CellRange(0, 0, p_Grid.Rows.Count - 1, p_Grid.FixedColumns);
		else
			return CellRange.Empty;
	}
}
...
public class RangeLoader : IRangeLoader {
	...
	/// Constructor
	/// </summary>
	public RangeLoader(CellRange range) {
		mRange = range;
	}
	...
	/// <param name="p_Grid"></param>
	/// <returns></returns>
	public virtual CellRange GetRange(GridVirtual p_Grid) {
		return mRange;
	}
}
