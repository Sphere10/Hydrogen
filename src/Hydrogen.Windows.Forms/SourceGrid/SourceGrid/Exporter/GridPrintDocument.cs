...
public class GridPrintDocument : PrintDocument {
	...
	private CellRange m_RangeToPrint = CellRange.Empty;

	public CellRange RangeToPrint {
	...
	protected override void OnBeginPrint(PrintEventArgs e) {
		...
		if (RangeToPrint.IsEmpty())
			RangeToPrint = new CellRange(0, 0, m_Grid.Rows.Count - 1, m_Grid.Columns.Count - 1);
	...
	protected override void OnPrintPage(PrintPageEventArgs e) {
		...
		this.DrawHeader(e.Graphics,
			new RectangleF(e.MarginBounds.Left, e.MarginBounds.Top, e.MarginBounds.Width, m_HeaderHeight),
			m_PageNo,
			m_PageCount);

		if (m_PageNo == 1)
			this.DrawTitle(e.Graphics,
				new RectangleF(e.MarginBounds.Left, e.MarginBounds.Top + m_HeaderHeight, e.MarginBounds.Width, m_TitleHeight),
				m_PageNo,
				m_PageCount);
		...
		while (m_NextRowToPrint <= RangeToPrint.End.Row) {
			...
			while (m_NextColumnToPrint <= pageLastColumn) {
				...
				CellRange range = m_Grid.PositionToCellRange(pos);
		...
		this.DrawFooter(e.Graphics,
			new RectangleF(e.MarginBounds.Left, e.MarginBounds.Bottom - m_FooterHeight, e.MarginBounds.Width, m_FooterHeight),
			m_PageNo,
			m_PageCount);
