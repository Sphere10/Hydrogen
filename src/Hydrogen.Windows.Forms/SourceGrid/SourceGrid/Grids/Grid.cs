...
public partial class Grid : GridVirtual {
	...
	public Grid() {
		...
		spannedCellReferences = new SpannedCellRangesController(
			this,
			new QuadTreeRangesList(CellRange.From(new Position(0, 0), 4, 4)));
	...
	/// <summary>
	/// Returns the maximum number of rows that can fit without a vertical scrollbar
	/// </summary>
	[Browsable(false), DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
	public int RowsPerPage {
		get { return base.VerticalPage; }
	}
	...
	public Cells.ICell this[Position position] {
		...
		set { InsertCell(position.Row, position.Column, value); }
	}
	...
	private void EnsureNoSpannedCellsExist(int row, int col, Cells.ICell p_cell) {
		var spanRange = new CellRange(row, col, row + p_cell.RowSpan - 1, col + p_cell.ColumnSpan - 1);
	...
	[Obsolete] private static int m_MaxSpan = 100;
	...
	/// </summary>
	/// <returns></returns>
	public override CellRange RangeToCellRange(CellRange range) {
		...
		for (int x2 = range.Start.Column; x2 <= range.End.Column; x2++) {
			for (int y2 = range.Start.Row; y2 <= range.End.Row; y2++) {
				...
				CellRange range2 = PositionToCellRange(p);
				if (range2.IsEmpty())
					range2 = new CellRange(p, p);
		...
		return new CellRange(y, x, y1, x1);
	}
	...
	/// <returns></returns>
	public override CellRange PositionToCellRange(Position pPosition) {
