//-----------------------------------------------------------------------
// <copyright file="QuadTreeNode.cs" company="Sphere 10 Software">
//
...
namespace QuadTreeLib
{
	...
	public class ProportioanteSizeNodeDivider : IQuadTreeNodeDivider
	{
		...
		public QuadTreeNode CreateNewRoot(QuadTreeNode currentRoot)
		{
			...
			var newRoot = new QuadTreeNode(
				new CellRange(startRow, startCol, halfCol * 2, halfRow * 2),
				currentRoot.Depth, currentRoot.QuadTree);
			
			
			newRoot.Nodes.Add(currentRoot);
			newRoot.Nodes.Add(new QuadTreeNode(CellRange.From(
		...
		private void CreateNotProportionate(QuadTreeNode parentNode)
		{
			...
			parentNode.Nodes.Add(new QuadTreeNode(CellRange.From(m_bounds.Start, halfRow, halfCol), Depth, QuadTree));
			parentNode.Nodes.Add(new QuadTreeNode(CellRange.From(
				new Position(startRow + halfRow, startCol),
				halfRow, halfCol), Depth, QuadTree));
		}
	...
	public class QuadTreeNode
	{
		...
		public QuadTreeNode(CellRange bounds)
		{
		...
		/// </summary>
		public QuadTreeNode(CellRange bounds, int currentDepth, QuadTree quadTree)
			:this(bounds)
		{
			m_bounds = bounds;
			QuadTree = quadTree;
		...
		/// </summary>
		List<CellRange> m_contents = new List<CellRange>();
		...
		/// </summary>
		public CellRange Bounds { get { return m_bounds; } }
		...
		/// </summary>
		public List<CellRange> SubTreeContents
		{
			get
			{
				List<CellRange> results = new List<CellRange>();
		...
		public List<CellRange> Contents { get { return m_contents; } }

		
		public List<CellRange> Query(CellRange queryArea)
		{
			return QueryInternal(queryArea, false);
		}
		
		public CellRange? QueryFirst(CellRange queryArea)
		{
		...
		/// </summary>
		/// <returns></returns>
		public List<CellRange> QueryInternal(CellRange queryArea, bool stopOnFirst)
		{
		...
		public List<CellRange> Query(Position queryArea)
		{
			// create a list of the items that are found
			List<CellRange> results = new List<CellRange>();
		...
		public CellRange? QueryFirst(Position queryArea)
		{
			// this quad contains items that are not entirely contained by
			// it's four sub-quads. Iterate through the items in this quad
			// to see if they intersect.
		...
		public bool Remove(CellRange range)
		{
		...
		public void Insert(CellRange item)
		{
			// if the item is not contained in this quad, there's a problem
