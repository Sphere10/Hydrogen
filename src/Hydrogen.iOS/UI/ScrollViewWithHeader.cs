//-----------------------------------------------------------------------
// <copyright file="ScrollViewWithHeader.cs" company="Sphere 10 Software">
//
...
namespace Hydrogen.iOS
{
	public class ScrollViewWithHeader : UIView
	{
		...
		public float ZoomScale { 
			get {
				return (float)_mainContent.ZoomScale;
			}			
			set {
		...
		public void SetupController(CGRect rect, UIView header, UIView rowHeader, UIView content, bool enableZoom)
		{
			...
			_mainContentDelegate.Scrolling += delegate
			{
				...
				if (!_mainContent.Zooming && !isZooming)
					scrollContent();
				this.ContentOffset =  (CGPoint)_mainContent.ContentOffset;
		...
		{
			CGPoint hOffSet = (CGPoint)_header.ContentOffset;
			CGPoint cOffSet = (CGPoint)_mainContent.ContentOffset;
			var rOffSet = new CGPoint(0, cOffSet.Y);
			if (_rowHeader != null)
				rOffSet = (CGPoint)_rowHeader.ContentOffset;
			
			if (cOffSet.X != hOffSet.X || rOffSet.Y != cOffSet.Y)
			{
				CGRect cFrame = (CGRect)_mainContent.Frame;
				cFrame.X = hOffSet.X;
				cFrame.Y = rOffSet.Y;
		...
		private void scrollHeader()
		{
			...
			{
				CGRect hFrame = (CGRect)_header.Frame;
				hFrame.X = cOffSet.X;
				hFrame.Y = hOffSet.Y;
			...
			{
				CGRect rFrame = (CGRect)_rowHeader.Frame;
				rFrame.X = rFrame.X;
				rFrame.Y = cOffSet.Y;
		...
		{
			float scale = (float)_mainContent.ZoomScale;
			if (scale != (float)_header.ZoomScale)
			{
				CGRect headerFrame = (CGRect)_header.Frame;
				headerFrame.Height = _headerFrame.Height*scale;
				
				CGRect rHeaderFrame = (CGRect)_rowHeader.Frame;
				...
				CGRect mainFrame = (CGRect)_mainContent.Frame;
