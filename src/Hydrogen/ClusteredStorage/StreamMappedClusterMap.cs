...
public class StreamMappedClusterMap : ClusterMap, ILoadable {
	...
	public override ClusterTraits ReadClusterTraits(long cluster) {
		_innerStreamPagedList.ReadItemBytes(cluster, Cluster.TraitsOffset, Cluster.TraitsLength, out var bytes);
	...
	protected override void WriteClusterTraitsInternal(long clusterIndex, ClusterTraits traits) {
		_innerStreamPagedList.WriteItemBytes(clusterIndex, Cluster.TraitsOffset, new[] { (byte)traits });
	}

	protected override void MaskClusterTraitsInternal(long clusterIndex, ClusterTraits traits, bool on) {
		_innerStreamPagedList.ReadItemBytes(clusterIndex, Cluster.TraitsOffset, Cluster.TraitsLength, out var traitBytes);
		var newTraits = ((ClusterTraits)traitBytes[0]).CopyAndSetFlags(traits, on);
		_innerStreamPagedList.WriteItemBytes(clusterIndex, Cluster.TraitsOffset, new[] { (byte)newTraits });
	}

	public override long ReadClusterPrev(long cluster) {
		_innerStreamPagedList.ReadItemBytes(cluster, Cluster.PrevOffset, Cluster.PrevLength, out var bytes);
	...
	protected override void WriteClusterPrevInternal(long clusterIndex, long prev) {
		...
		_innerStreamPagedList.WriteItemBytes(clusterIndex, Cluster.PrevOffset, bytes);
	}

	public override long ReadClusterNext(long cluster) {
		_innerStreamPagedList.ReadItemBytes(cluster, Cluster.NextOffset, Cluster.NextLength, out var bytes);
	...
	protected override void WriteClusterNextInternal(long clusterIndex, long next) {
		...
		_innerStreamPagedList.WriteItemBytes(clusterIndex, Cluster.NextOffset, bytes);
	}

	public override byte[] ReadClusterData(long clusterIndex, long offset, long size) {
		_innerStreamPagedList.ReadItemBytes(clusterIndex, Cluster.DataOffset + offset, size, out var bytes);
		return bytes;
	}
	...
	protected override void WriteClusterDataInternal(long clusterIndex, long offset, ReadOnlySpan<byte> data) {
		...
		_innerStreamPagedList.WriteItemBytes(clusterIndex, Cluster.DataOffset + offset, data);
	}
}