...
public static class TopologicalSorter {
	...
	public static IList<IList<TSource>> FindAndTopologicallySortStronglyConnectedComponents<TSource, TVertex>(IEnumerable<TSource> source, Func<TSource, TVertex> getVertex, IEnumerable<Tuple<TVertex, TVertex>> edges,
	...
	public static IEnumerable<T> TopologicalSort<T>(IEnumerable<T> source, Func<T, IEnumerable<T>> getEdges, IEqualityComparer<T> comparer = null) {
		var sourceArray = source as T[] ?? source.ToArray();
		var edges =
			from item in sourceArray
			from dependency in getEdges(item)
			select Tuple.Create(item, dependency);

		var result = FindAndTopologicallySortStronglyConnectedComponents(sourceArray, edges, comparer);
		if (result.Any(x => x.Count > 1))
			throw new InvalidOperationException("Cycles in graph");
		return result.Select(x => x[0]);
	}
