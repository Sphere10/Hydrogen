...
public record HydrogenFileDescriptor : TransactionalFileDescriptor {
	...
	public static HydrogenFileDescriptor From(string path, string pagesDirectoryPath, long pageSize = HydrogenDefaults.TransactionalPageSize, long maxMemory = HydrogenDefaults.MaxMemoryPerCollection, int clusterSize = HydrogenDefaults.ClusterSize, ClusteredStreamsPolicy containerPolicy = HydrogenDefaults.ContainerPolicy) 
		=> new() { 
			Path = path,
