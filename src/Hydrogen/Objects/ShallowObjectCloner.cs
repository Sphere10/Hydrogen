...
namespace Hydrogen {
	...
	public class ShallowObjectCloner : IObjectCloner {
		...
		public virtual void Copy(object source, object dest) {
			ShallowCopyMembers(source, dest);
		}

		protected virtual object ShallowClone(object source) {
			var dest = Tools.Object.Create(source.GetType());
			ShallowCopyMembers(source, dest);
			return dest;
		}

		protected virtual void ShallowCopyMembers(object source, object dest) {
