...
internal class CyclicReferenceAwareSerializer<T> : ItemSerializerDecorator<T> {
	...
	public override long CalculateTotalSize(IEnumerable<T> items, bool calculateIndividualItems, out long[] itemSizes) {
		...
		foreach (var item in items) {
			var itemSize = CalculateSize(item);
	...
	public override long CalculateSize(T item) {
		...
		if (scope.HasSerializedObject(item, out var serializationContextIndex)) {
			return _cyclicReferenceMarkerSizeBytes + CyclicReferenceSerializer.Instance.CalculateSize(new CyclicReference(serializationContextIndex));
		}
		scope.NotifySerializingObject(item, true);
		return base.CalculateSize(item);
	...
	public override void Serialize(T item, EndianBinaryWriter writer) {
		...
		if (scope.HasSerializedObject(item, out var serializationContextIndex)) {
			...
			CyclicReferenceSerializer.Instance.Serialize(new CyclicReference(serializationContextIndex), writer);
		} else {
	...
	public override T Deserialize(EndianBinaryReader reader) {
		...
		if (_peekForCyclicReferenceMarker(reader)) {
			...
			var cyclicReference = CyclicReferenceSerializer.Instance.Deserialize(reader);
...
internal class CyclicReferenceSerializer : ItemSerializer<CyclicReference> {
	...
	public override long CalculateSize(CyclicReference item)
		=> CVarInt.SizeOf(unchecked((ulong)item.Index));

	public override void Serialize(CyclicReference item, EndianBinaryWriter writer)
		=> CVarInt.Write(unchecked((ulong)item.Index), writer.BaseStream);

	public override CyclicReference Deserialize(EndianBinaryReader reader)
