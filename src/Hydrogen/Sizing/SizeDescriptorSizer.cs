...
public class SizeDescriptorSizer : ItemSizerBase<long> {
	...
	public override bool IsStaticSize => SizeDescriptorStrategy == SizeDescriptorStrategy.UseULong;

	public override long StaticSize => IsStaticSize ? sizeof(ulong) : throw new InvalidOperationException($"Size is not statically sized for {SizeDescriptorStrategy}");
