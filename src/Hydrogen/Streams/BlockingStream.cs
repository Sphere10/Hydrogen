//-----------------------------------------------------------------------
// <copyright file="BlockingStream.cs" company="Sphere 10 Software">
//
...
namespace Hydrogen {
	public class BlockingStream : Stream {
		...
		public override int Read(byte[] buffer, int offset, int count) {
			...
			while (true) {
				...
				lock (_lockForRead) {
					...
					var bytesAvailable = _currentChunk.Length - _currentChunkPosition;
					int bytesToCopy;
		...
		internal void SetEndOfStreamDueToFailure() {
			try {
				lock (_lockForAll) {
					_illegalToWrite = true;
					_doneWriting.Set();
