//-----------------------------------------------------------------------
// <copyright file="SqliteDAC.cs" company="Sphere 10 Software">
//
...
//CascadeDelete	BIT NOT NULLABLE
		    ...
		    using (var scope = this.BeginScope()) {
		        ...
		        var sequences =
		            sqliteMasterTables.Any(x => x.name == "sqlite_sequence") ?
		                SelectSqliteSequence().ToDictionary(s => s.name) :
		                new Dictionary<string, sqlite_sequence_result>();
