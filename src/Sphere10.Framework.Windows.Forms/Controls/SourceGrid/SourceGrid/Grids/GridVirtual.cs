//-----------------------------------------------------------------------
// <copyright file="GridVirtual.cs" company="Sphere 10 Software">
//
...
namespace SourceGrid
{
	...
	public abstract class GridVirtual : CustomScrollControl
	{
		...
		public virtual void AutoSizeCells(CellRange p_RangeToAutoSize)
		{
			SuspendLayout();
		...
		{
			CellRange complete = CompleteRange;
		...
		public Size RangeToSize(CellRange range)
		{
			if (range.IsEmpty())
				return Size.Empty;
		...
		/// </summary>
		public Rectangle RangeToRectangle(CellRange range)
		{
			if (range.IsEmpty())
		...
		public CellRange RangeAtArea(CellPositionType areaType)
		{
			...
			else if (areaType == CellPositionType.FixedLeft)
			{
				...
				return new CellRange(firstRow.Value, 0, lastRow.Value, actualFixed - 1);
			}
			else if (areaType == CellPositionType.FixedTop)
			{
				...
				return new CellRange(0, firstCol.Value, actualFixed - 1, lastCol.Value);
			}
			else if (areaType == CellPositionType.Scrollable)
			{
				...
				return new CellRange(firstRow.Value, firstCol.Value,
		...
		///// </summary>
		//public CellRange RangeAtRectangle(CellPositionType areaType, Rectangle clipRectangle)
		//{
		//    if (areaType == CellPositionType.FixedTopLeft)
		//    {
		//    }
		...
		/// </summary>
		/// <returns></returns>
		private IEnumerable<CellRange> GetVisibleRegion()
		{
			CellRange rng;
		...
		///// <returns></returns>
		//public bool IsRangeVisible(CellRange range, bool partial)
		//{
		...
		{
			InvalidateRange(new CellRange(position));
		}
		...
		/// </summary>
		/// <param name="range"></param>
		public void InvalidateRange(CellRange range)
		{
			if (range.IsEmpty())
				return;
			...
			{
				CellRange visibleRange = RangeAtArea(type);
				// clip our range with visible range
				// so that we wount loop through thousands of rows
				CellRange clippedRange = CellRange.Intersect(range, visibleRange);
		...
		/// <returns></returns>
		public CellRange RangeAtAreaExpanded(CellPositionType areaType)
		{
			CellRange range = RangeAtArea(areaType);
		...
		/// </summary>
		/// <returns></returns>
		public virtual CellRange RangeToCellRange(CellRange range)
		{
			return new CellRange(range.Start, range.End);
		}
		...
		/// <returns></returns>
		public virtual CellRange PositionToCellRange(Position pPosition)
		{
			...
			if (l_Cell == null)
				...
				return new CellRange(pPosition);
			//return new CellRange(pPosition);
		}
		#endregion
		...
		protected virtual Selection.SelectionBase CreateSelectionObject()
		{
			switch (SelectionMode)
			{
				case GridSelectionMode.None:
					return new Selection.DisabledSelection();
		...
		{
			m_OldMouseSelectionRange = CellRange.Empty;
			if (MouseSelectionFinished != null) {
				MouseSelectionFinished(this, e);
			}
		}

		private CellRange m_OldMouseSelectionRange = CellRange.Empty;
		private CellRange m_MouseSelectionRange = CellRange.Empty;
		/// <summary>
		...
		/// </summary>
		[Browsable(false),DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		public virtual CellRange MouseSelectionRange
		{
		...
		{
			CellRange l_MouseRange = MouseSelectionRange;
		...
		{
			if (m_MouseSelectionRange != CellRange.Empty)
				OnMouseSelectionFinish(new RangeEventArgs(m_OldMouseSelectionRange));

			m_MouseSelectionRange = CellRange.Empty;
		}
		...
		{
			CellRange newMouseSelection = new CellRange(Selection.ActivePosition, p_Corner);
		...
		public virtual void ProcessSpecialGridKey(KeyEventArgs e)
		{
			...
			if (shiftPressed)
			{
				...
				Selection.SelectRange(new CellRange(m_firstCellShiftSelected, Selection.ActivePosition), true);
			}
		...
		public void PerformPaste(RangeRegion selRegion)
		{
			...
			if (rngData == null)
				return;
			CellRange rng = selRegion[0];
			CellRange data = rngData.SourceRange;
			CellRange destinationRange = new CellRange(
		...
		public virtual void ArrangeLinkedControls()
		{
			...
			foreach (LinkedControlValue linked in m_LinkedControls)
			{
				...
				{
					CellRange scrollableRange = RangeAtArea(CellPositionType.Scrollable );
		...
		{
			CellRange l_Range = p_RangeToSort.GetRange(this);
		...
		public void SortRangeRows(CellRange p_Range,
		                          int keyColumn,
		                          bool p_bAscending,
		                          IComparer p_CellComparer)
		{
		...
		/// </summary>
		[Browsable(false),DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		public CellRange CompleteRange
		{
			get
			{
				if (Rows.Count > 0 && Columns.Count > 0)
					return new CellRange(0, 0, Rows.Count - 1, Columns.Count - 1);
				else
					return CellRange.Empty;
			}
		}
...
//		http://www.microsoft.com/middleeast/msdn/mirror.aspx
		...
		public override void CustomScrollPageDown()
		{
			...
			CellRange rngFocusedCell = PositionToCellRange(Selection.ActivePosition);
