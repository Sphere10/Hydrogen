...
namespace Sphere10.Framework {
	...
	/// <summary>
	/// An extended list performance optimized for bytes. Useful for dealing with file downloads, image manipulation, etc.
	/// <remarks>
	/// All enumerable results are implemented as arrays and arguments are not checked for performance.
	/// </remarks>
	/// </summary>
	public sealed class MemoryBuffer : RangedListBase<byte> {
		...
		public Span<byte> AsSpan(int index) {
			return _internalArray.AsSpan(index);
		}
		...
		public Span<byte> AsSpan(Index index) {
			return _internalArray.AsSpan(index);
		}

		public Span<byte> AsSpan(Range range) {
			return _internalArray.AsSpan(range);
		}
