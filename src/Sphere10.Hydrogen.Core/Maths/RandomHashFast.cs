...
namespace Sphere10.Hydrogen.Core.Maths {
	public sealed class RandomHashFast {
		...
		private readonly CHF[] _hashAlg = new CHF[18];
		...
		private void MemTransform1(ref byte[] aBuffer, int aReadStart, int aWriteStart, int aLength) {
			...
			for (int i = aWriteStart; i <= writeEnd; i++) {
				aBuffer[i] = aBuffer[aReadStart + (XorShift.Next(ref state) % aLength)];
			}
		}
		...
		private void MemTransform7(ref byte[] aBuffer, int aReadStart, int aWriteStart, int aLength) {
			...
			for (int i = 0; i < aLength; i++) {
				aBuffer[aWriteStart + i] = RandomHashUtils.RotateLeft8(aBuffer[aReadStart + i], aLength - i);
			}
		}
		...
		private void MemTransform8(ref byte[] aBuffer, int aReadStart, int aWriteStart, int aLength) {
			...
			for (int i = 0; i < aLength; i++) {
				aBuffer[aWriteStart + i] = RandomHashUtils.RotateRight8(aBuffer[aReadStart + i], aLength - i);
			}
		}
		...
		private byte[] Hash(byte[] aBlockHeader) {
			...
			return Hashers.Hash(_hashAlg[0], Compress(allOutputs));
		}
		...
		private ChecksummedByteCollection Hash(byte[] aBlockHeader, int aRound) {
			...
			byte[] output = Hashers.Hash(_hashAlg[gen.NextUInt32() % 18], roundInput);
		...
		public RandomHashFast() {
			...
			_hashAlg[0] = CHF.SHA2_256;
			_hashAlg[1] = CHF.SHA2_384;
			_hashAlg[2] = CHF.SHA2_512;
			_hashAlg[3] = CHF.SHA3_256;
			_hashAlg[4] = CHF.SHA3_384;
			_hashAlg[5] = CHF.SHA3_512;
			_hashAlg[6] = CHF.RIPEMD_160;
			_hashAlg[7] = CHF.RIPEMD_256;
			_hashAlg[8] = CHF.RIPEMD_320;
			_hashAlg[9] = CHF.Blake2b_512;
			_hashAlg[10] = CHF.Blake2s_256;
			_hashAlg[11] = CHF.Tiger2_5_192;
			_hashAlg[12] = CHF.Snefru_8_256;
			_hashAlg[13] = CHF.Grindahl512;
			_hashAlg[14] = CHF.Haval_5_256;
			_hashAlg[15] = CHF.MD5;
			_hashAlg[16] = CHF.RadioGatun32;
			_hashAlg[17] = CHF.WhirlPool;
		}
