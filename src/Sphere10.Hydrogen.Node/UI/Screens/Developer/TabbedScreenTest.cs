	...
	public class TabbedScreenTest : TabbedScreen<TabbedScreenTest.Model> {
		...
		public class LogScreen : Screen<Model> {
			...
			protected override void OnAppearing() {
				...
				_timer ??= new Timer(
					state => {
						_outputView?.AppendLog($"OnTimer {++_count} {Guid.NewGuid()}");
					},
					null,
					TimeSpan.Zero,
		...
		public class OptionsScreen : FramedScreen<Model> {
			...
			protected override void LoadInternal() {
				...
				labelFieldLayout.AddButton("Enum Selection", "Button Label", () => {
		...
		public enum EnumOption {
			[Description("Option 1 text")]
			Option1,

			[Description("Option 2 text")]
			Option2,

			[Description("Option 3 text")]
			Option3,
