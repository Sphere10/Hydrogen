...
//		if (!_int64Serializer.TrySerialize(item.PID, writer, out var int64BytesWritten))
//			return false;

//		if (!_dateTimeSerializer.TrySerialize(item.FirstSeen, writer, out var dateTimeBytesWritten1))
//			return false;

//		if (!_dateTimeSerializer.TrySerialize(item.LastSeen, writer, out var dateTimeBytesWritten))
//			return false;

//		bytesWritten = stringBytesWritten + int64BytesWritten + dateTimeBytesWritten1 + dateTimeBytesWritten;
//		return true;
//	}
...
//		if (!_int64Serializer.TryDeserialize(reader, out var pPPID))
//			return false;

//		if (!_dateTimeSerializer.TryDeserialize(sizeof(long), reader, out var firstSeen))
//			return false;

//		if (!_dateTimeSerializer.TryDeserialize(sizeof(long), reader, out var lastSeen))
//			return false;

//		item = new PlayerRecord {
//			TID = TID,
//			PID = pPPID,
//			FirstSeen = firstSeen,
//			LastSeen = lastSeen,
