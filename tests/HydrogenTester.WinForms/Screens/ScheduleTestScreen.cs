...
namespace Sphere10.FrameworkTester.WinForms
{
    public partial class ScheduleTestScreen : ApplicationScreen {
        ...
        private void _job1Button_Click(object sender, EventArgs e) {
            ...
            Sphere10.Framework.Scheduler.Scheduler.Asynchronous.JobStatusChanged += 
                (job, fromStatus, toStatus) => _textWriter.WriteLine("{0}: {1} -> {2}", job.Name, fromStatus, toStatus);
            Sphere10.Framework.Scheduler.Scheduler.Asynchronous.AddJob(job1);
            Sphere10.Framework.Scheduler.Scheduler.Asynchronous.AddJob(job2);
        }
